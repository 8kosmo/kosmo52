 ...
 * 이때 파라미터는 반드시 request, response사용함.
 * 그래야 그 객체를 주입받으므로 NullPointerExcepton피할 수 있다.
 * 서블릿은 톰캣 서버에서 싱글톤으로 객체를 관리하므로 한개의 서블릿 클래스를 메모리에 올린  후
 * 여러 사용자가 사용할 땐 스레드를 통하여 순서 혹은 경합의 경우 공평하게 서비스를 제공받게 됨.
 * 싱글톤으로 만들어진 RequestTest객체를 톰캣에서 관리하므로 내가 직접 예외처리하는데 한계가
 * 있다.
 */
	@Override
	...
	throws ServletException, IOException
	{
		 ...
		 */
		//res.sendRedirect("requestTestResult.jsp");
		RequestDispatcher view = 
				req.getRequestDispatcher("./requestTestResult.jsp");
		//이동이 일어남
