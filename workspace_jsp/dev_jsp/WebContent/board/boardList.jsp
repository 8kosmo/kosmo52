...
*/
	function boardList(){//태그 조작(DOM조작 - json:DefaultTableModel datagrid: JTable)
		...
		//alert(tb_value);
		$("#dg_board").datagrid({
			url:"test.mo?crud=boardList&cb_search="+cb_value
					  +"&keyword="+tb_value
					  +"&pageNumber=1"
					  +"&pageSize=2"
		   //,pagination:'true'		  
 		   ,onLoadSuccess:function(data){
			   //alert("load success!!"+data.total);
 			   $.ajax({
				   url:"test.mo?crud=total&cb_search="+cb_value
					  +"&keyword="+tb_value+"&timestamp="+new Date().getTime()
				  ,method:"get"		   
...
/* =========== [[datagrid 초기화 및 설정]] ============*/		
		$("#dg_board").datagrid({
		    ...
		    columns:[[
		        ...
		        {field:'BM_DATE',title:'조회수',width:170,align:'center'}
		    ]]
		    ...
		    ,onDblClickCell: function(index,field,value){
		   			...
		   			if("BM_TITLE" == field){
		   				//alert("제목"+value);
		   				cmm_window_popup("test.mo?crud=boardDetail&bm_no="+g_no,"700","450","read");
		   				//선택한 로우값 초기화
