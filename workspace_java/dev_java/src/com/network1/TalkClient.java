...
public class TalkClient extends JFrame implements ActionListener{
	...
	public void initDisplay() {
		...
		this.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent we) {
				try {
		//윈도우창을 닫거나 컴터를 강제로 종료시켰을 때도 호출되도록 하자.			
					exitChat();
		//아래코드는 자바가상머신과의 연결고리를 강제로 끊어서 기존에 생성된 객체가 candidate
		//상태로 빠지도록 유도함.
		//그러면  JVM이 candidate상태에 놓인 클래스들을 뽑아서 가비지 컬렉터가 쓰레기값으로
		//메모리를 회수하고 할당되어 있던 스레드도 회수해감.			
					System.exit(0);
				} catch (Exception e) {
	...
	public void message_process(String msg, String imgChoice) {//200|닉네임|메세지
		System.out.println("message_process 호출 성공");
		//이모티콘 메시지를 전송
		if(imgChoice!=null) {//이모티콘을 보낼거니?
			...
			try {
				oos.writeObject(Protocol.MESSAGE//200
						+ Protocol.seperator+nickName//닉네임
						+ Protocol.seperator+msg//메세지
						+ Protocol.seperator+imgChoice//선택한 이모티콘 정보 넘김
						+ Protocol.seperator+fontColor//글자색
						);				
			} catch (Exception e) {
				e.printStackTrace();
			}
