		...
		$.messager.confirm('Confirm','정말삭제하시겠습니까?',function(r){
			if(r){
				...
				for(var j=0;j<isOk;j++){
					//필드명은 myBatis의 Map을 사용하였으므로 반드시 대문자로 처리함.
					bsfileArr.push(rows[j].BS_FILE2);
				}
				//배열에 담긴 정보 출력해보기
		 		for(var i=0;i<rows.length;i++){
					...
					//자바코드에서 StringTokenizer로 문자열을 썰때 ,를 기준으로 
					//썰어서 담으니까 맨 끝에 콤마가 있으면 실제 있는값보다 1개가 더 생김
		 			else{
		 				vbmnos +=bmnoArr[i]+",";
		 			}
				}//////////////////////end of for
				//로우의 수는 고정이지만 첨부파일은 있는 경우도 있고 없는 경우도 있으므로
				//똑같이 반복되어서는 안될 것임.
				//배열에 담기는  글번호의 갯수와 첨부 파일의 갯수가 다를 수 있다는 것임.
