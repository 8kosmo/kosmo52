...
//수정
	function updateForm(){
		cmm_window_popup('/board/test.mo?crud=updateForm&bm_no=<%=rMap.get("BM_NO")%>','700','470','updaeForm');
	}
//삭제화면 열기
	function deleteForm(){
		$("#dlg_boardDel").dialog({
    	...
    	*/
    	if(db_pw == u_pw){//비번이 일치했을때 삭제 프로세스를 태운다.
    		...
    		$.messager.confirm('Confirm','정말 삭제하겠습니까?',function(r){
    			if(r){
    				location.href="/board/test.mo?crud=boardDel&bm_no=<%=rMap.get("BM_NO")%>&bs_file=<%=rMap.get("BS_FILE")%>";
    			}
    		});
...
//목록
	function boardList(){
		location.href="/board/test.mo?crud=boardList";
    }
</script>
</head>
<body>
